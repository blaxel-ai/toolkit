...
type Resource struct {
	...
	MetadataURL    string // URL from API response metadata
	mu             sync.RWMutex
}
	...
	if m.complete {
		...
		if allSuccess {
			...
			if core.IsVolumeTemplate(config.Type) {
				...
				// Read metadata URL from resource
				m.resources[0].mu.RLock()
				metadataURL := m.resources[0].MetadataURL
...
// SetMetadataURL sets the metadata URL for a resource
func (r *Resource) SetMetadataURL(url string) {
	if r != nil {
		r.mu.Lock()
		r.MetadataURL = url
		r.mu.Unlock()
