...
func TestDeployedStatusIsFinal(t *testing.T) {
	tests := []struct {
		...
		{"skip-build accepts DEPLOYED immediately", false, false, true},
		{"build: stale DEPLOYED before any rollout status is ignored", true, false, false},
		{"build: DEPLOYED after BUILDING or DEPLOYING is final", true, true, true},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := deployedStatusIsFinal(tt.autoGenerated, tt.sawRolloutStatus); got != tt.want {
				t.Fatalf("deployedStatusIsFinal(%v, %v) = %v, want %v", tt.autoGenerated, tt.sawRolloutStatus, got, tt.want)
			}
		})
	}
}
