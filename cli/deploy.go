...
// reached DEPLOYED is a revision newer than the one it was on before the apply:
// the previous deployment's DEPLOYED, or a previous rollout still in flight
// when the apply happened, must not end the wait. When revisions are not
...
func deployedStatusIsFinal(autoGenerated bool, sawRolloutStatus bool, baseline rolloutBaseline, deployedRevision string) bool {
	...
	if baseline.known && deployedRevision != "" {
		return deployedRevision != baseline.revision
	}
...
// rolloutBaseline is the revision a resource was on before this deploy was
// applied. known is false when the resource could not be read at that point.
...
func readRolloutBaseline(resourceType, name string) rolloutBaseline {
	...
	return rolloutBaseline{revision: rollout.LatestRevision, known: true}
}
