...
func DeployCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			core.SetInteractiveMode(interactive)
			if recursive {
			...
			if interactive {
				err = deployment.ApplyInteractive(mockMode)
			} else {
	...
	cmd.Flags().BoolVarP(&interactive, "interactive", "i", false, "Enable interactive deployment mode with live status and build logs")
	cmd.Flags().BoolVarP(&mockMode, "mock", "", false, "Use mock mode for testing interactive deployment (only works with --interactive)")
	return cmd
}
...
func (d *Deployment) ApplyInteractive(mockMode bool) error {
	...
	if mockMode {
		// In mock mode, add sample resources of different types
		additionalResources = append(additionalResources,
			...
			&deploy.Resource{
				...
				Status:        deploy.StatusPending,
				AutoGenerated: true, // This will show build logs
			},
			&deploy.Resource{
				Kind:          "Job",
				Name:          "data-processor",
			...
			&deploy.Resource{
				...
				Name:          "dev-sandbox",
				Status:        deploy.StatusPending,
				AutoGenerated: true, // Another resource with build logs
			},
		)
		resources = append(resources, additionalResources...)
	} else if _, err := os.Stat(blaxelDir); err == nil {
	...
	go d.runInteractiveDeployment(resources, additionalResources, model, mockMode)
...
func (d *Deployment) runInteractiveDeployment(resources []*deploy.Resource, additionalResources []*deploy.Resource, model *deploy.InteractiveModel, mockMode bool) {
	...
	for i := mainResourceCount; i < len(resources); i++ {
		...
		go func(idx int) {
			...
			if mockMode {
				...
				if resource.AutoGenerated {
					// Simulate build for auto-generated resources
					...
					// Mock build logs based on resource type
					var logWatcher interface {
						Start()
						Stop()
					}
					switch strings.ToLower(resource.Kind) {
					case "function":
						logWatcher = deploy.NewMockBuildLogWatcherWithType(func(log string) {
					...
					default:
						logWatcher = deploy.NewMockBuildLogWatcher(func(log string) {
							model.AddBuildLog(idx, log)
						})
					}
					logWatcher.Start()
	...
	for i := 0; i < mainResourceCount; i++ {
		...
		go func(idx int, depl core.Result) {
			defer wg.Done()
			d.deployResourceInteractive(resources[idx], model, idx, depl, mockMode)
...
func (d *Deployment) deployResourceInteractive(resource *deploy.Resource, model *deploy.InteractiveModel, idx int, deployment core.Result, mockMode bool) {
	// Start deployment
	model.UpdateResource(idx, deploy.StatusDeploying, "Applying resource", nil)
	model.AddBuildLog(idx, fmt.Sprintf("Starting deployment of %s/%s", resource.Kind, resource.Name))

	if mockMode {
		// Mock deployment simulation with more realistic timing
		time.Sleep(2 * time.Second)
		...
		if resource.AutoGenerated {
			...
			// Simulate build with logs
			model.UpdateResource(idx, deploy.StatusBuilding, "Building image", nil)

			// Start mock log watcher
			logWatcher := deploy.NewMockBuildLogWatcher(func(log string) {
				model.AddBuildLog(idx, log)
			})
			logWatcher.Start()

			// Wait for mock build to complete - increased time to see logs
			...
			// Non-auto-generated resources deploy faster
			time.Sleep(3 * time.Second)
		}
