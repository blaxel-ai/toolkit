...
// the CLI triggered a build, a DEPLOYED read before any rollout status was
// observed is the stale status of the previous deployment and is ignored.
// Either BUILDING or DEPLOYING counts as the rollout: a build served from the
// cache leaves BUILDING within seconds, so a 3s poll may only see DEPLOYING.
func deployedStatusIsFinal(autoGenerated bool, sawRolloutStatus bool) bool {
	return !autoGenerated || sawRolloutStatus
}
		...
		for {
			...
			case <-statusTicker.C:
				...
				if status != lastStatus {
					...
					case "DEPLOYED":
						if !deployedStatusIsFinal(resource.AutoGenerated, sawRolloutStatus) {
							continue
						}
						if logWatcher != nil {
							logWatcher.Stop()
